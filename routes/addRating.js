...
function addRating(req, res) {
    ...
    var emp_no =  req.body.employeeno;
    ...
    pool.getConnection(function (err, connection) {
       ...
       // connection.query("Insert into `employees`.emp_ratings values ?" [metricValues],
        connection.query("INSERT INTO `employees`.emp_ratings(EmpNo,PunctualityAndDiscipline,ExecutionOfDuties,LearningAndDevelopment,TeamCooperation,ResponsibilityTaken) values (?,?,?,?,?,?)", [emp_no,metric1,metric2,metric3,metric4,metric5],
            function (err, result) {
            connection.release();
            if (err) {
